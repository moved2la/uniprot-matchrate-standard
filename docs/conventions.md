...
Generated by `build_protein_set.py`. Rule: master range = last UniProt `Chain` feature;
residues outside it are `false`, one segment per span, named `met1` for residue 1 when
it is an annotated initiator Met, otherwise `nterm_<a>_<b>` / `cterm_<a>_<b>`. Isoforms
inherit the canonical's N-terminal removal iff sequence-identical over the removed span
(checked from the FASTAs); the decisions file can override with `processing = none` or
`processing = master:<start>-<end>`. No coordinate is typed by hand.
...
- `config/` — the human layer. `protein_set_decisions.ini` and (later) the mass-fraction
  and fiber-mix files are hand-written and audited. `accessions.ini` and `segments.ini`
  are GENERATED by `build_protein_set.py` from the decisions plus `data/`; never edit
  them, and `pytest` fails if they are not exactly what the build produces.
