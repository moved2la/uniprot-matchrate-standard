...
def main() -> int:
    ...
    for gene in cands.sections():
        ...
        (raw / f"{gene}.json").write_text(json.dumps(data, indent=1), encoding="utf-8")
        ...
        for i, iso in enumerate(h["isoforms"], 1):
            ...
            if args.no_isoforms or not iso["ids"] or iso["status"] == "Not described":
                continue
            iso_id = iso["ids"][0]
            try:
    ...
    with open(outdir / "uniprot_verification.ini", "w", encoding="utf-8") as fh:
